...
class MI_Parser
{
    ...
    public static function validate_document($content)
    {
        ...
        if (count($lines) < 6) {
            $errors[] = __('File must have at least 6 header lines (release, visibility, meta, slug, title) plus markdown body.', 'markdown-importer');
        }
        ...
        $slug_line = trim(isset($lines[4]) ? (string) $lines[4] : '');
        $slug_error = '';
